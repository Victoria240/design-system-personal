...
- **Avatar DS component in sidebar context**: The `Avatar` component uses `after:mix-blend-darken` overlay and Base UI Fallback has a render delay — both conflict with the sidebar's token surface. Never use the DS `<Avatar>` inside the sidebar. Use a custom div matching the DS Avatar's `after:` border technique — NOT `ring-1` (which is an outer box-shadow and looks visually different). Footer avatar (size-6): `<div className="relative flex size-6 items-center justify-center rounded-full bg-sidebar-foreground/20 text-[10px] font-semibold text-sidebar-foreground select-none after:absolute after:inset-0 after:rounded-full after:border after:border-sidebar-border after:mix-blend-darken dark:after:mix-blend-lighten">VT</div>`. DM row avatars use the same pattern at `size-5` with `bg-sidebar-foreground/10`. The `ring-*` utility applies an outer box-shadow whereas the DS Avatar uses an inner pseudo-element border — these look different at the same token; always use the `after:` approach.

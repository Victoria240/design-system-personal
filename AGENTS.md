...
- **Avatar DS component in sidebar context**: The `Avatar` component uses `after:mix-blend-darken` overlay and Base UI Fallback has a render delay — both conflict with the sidebar's token surface. Never use the DS `<Avatar>` inside the sidebar. Use a custom div: `<div className="flex size-6 items-center justify-center rounded-full bg-sidebar-foreground/20 text-[10px] font-semibold text-sidebar-foreground select-none">VT</div>`.
